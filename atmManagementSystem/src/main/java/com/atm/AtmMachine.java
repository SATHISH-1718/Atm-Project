...
import com.notification.ShortMessageService;
/**
...
public class AtmMachine {
	...
	public void withdraw(Card card,short type) {
		...
		try {
					...
					if(amt<=10000) {
						...
						if((amt%100)==0) {
							...
							if(amt<=card.getBalance()+((type==1)?0:10000)){
								...
								notification=new ShortMessageService();
								notification.sendMessage(card,temp,sdate,ftime,"Debit");
			...
			if(recept=='y') {
				...
				System.out.println("|  Amount          : ₹" + String.format("%.2f", temp) + "              |");
	...
	public void deposite(Card card) {
		...
		try {
			...
			db.updateATM(amount,atmId);
			notification=new ShortMessageService();
			notification.sendMessage(card,amt,sdate,ftime,"Credit");
	...
	public void pinChange(Card card) {
		 Random random = new Random();
	        ...
	        try (BufferedWriter writer = new BufferedWriter(new FileWriter("notification.txt"))) {
	        	String otpmessage="PIN resetting OTP for your A/c XXXX"+card.getCardNumber()%10000+" is "
	        			+otp+" and it is valid for 60 seconds";
	            writer.write(otpmessage);
	            writer.close();
	            System.out.println("Pin resetting OTP has send to your registred phone number ..");
	        } catch (IOException e) {
	...
	public void fundtransfer(Card cardT,Card cardR){
		...
		try {
			...
			if(amt<=cardT.getBalance()) {
				...
				notification=new ShortMessageService();
				notification.sendMessage(cardT,amt,sdate,ftime,"Debit");
				System.out.println("Fund transferes Successfully..");
			}
			else {
				throw new BankException("Insufficient Balance ..! ");
			}
